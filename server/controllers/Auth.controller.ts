...
const authLogin = async (request: Request, response: Response) => {
    try {
        ...
        return response.status(requestResponse.status || 200).cookie("token", requestResponse.data.token, {sameSite: "none", secure: true, maxAge: 3600000, domain: "https://sage-panda-aa3ef1.netlify.app", httpOnly: true  }).json(requestResponse); 
    } catch (error) {
